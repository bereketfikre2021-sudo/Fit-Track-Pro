...
/** Months between two calendar dates (fractional). */
...
/**
 * Derive experience level from registration date.
 * 0–6 months: beginner, 6 months–2 years: intermediate, 2+ years: advanced.
 */
export function deriveFitnessLevelFromRegistration(registrationDate, refDate = new Date()) {
  if (!registrationDate || typeof registrationDate !== 'string') return 'beginner'
  const [y, m, d] = registrationDate.split('-').map(Number)
  if (!y || !m || !d) return 'beginner'
  const registered = new Date(y, m - 1, d)
  if (Number.isNaN(registered.getTime())) return 'beginner'

  const months = monthsBetween(registered, refDate)
...
/**
 * Effective fitness level: manual onboarding choice when set, otherwise auto-detected from registration.
 */
export function resolveEffectiveFitnessLevel(profile = {}, refDate = new Date()) {
  if (profile.fitnessLevelManual && FITNESS_LEVELS.has(profile.fitnessLevel)) {
    return profile.fitnessLevel
  }
  return deriveFitnessLevelFromRegistration(profile.registrationDate, refDate)
}
